...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // CUSTOM ADAPTATION: Marble & Gold tint
    // Original Output
    // return vec4(buf[0].x , buf[0].y , buf[0].z, 1.0);
    
    // Marble Color (White/Cream)
    vec3 marble = vec3(0.98, 0.98, 0.98);
    // Gold Color (Satin Gold #C5A059 => RGB: 197/255, 160/255, 89/255)
    vec3 gold = vec3(0.772, 0.627, 0.349);
    
    // Use the neural output to mix between Marble and Gold
    // We average the RGB channels to get an intensity
    float intensity = (buf[0].x + buf[0].y + buf[0].z) / 3.0;
    
    // Increase contrast to make the gold pop
    intensity = smoothstep(0.3, 0.7, intensity);
    
    vec3 finalColor = mix(marble, gold, intensity);
...
function ShaderBackground() {
    ...
    return (
        <div ref={canvasRef} className="bg-background absolute inset-0 -z-10 w-full h-full" aria-hidden>
            ...
            {/* Updated Overlay: Gold/White gradient instead of Black/Transparent */}
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-background/80 via-transparent to-background/50" />
        </div>
    );
}
    ...
    return (
        <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden bg-background">
            ...
            <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
                <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-none border border-primary/20 bg-primary/5 px-3 py-1.5 backdrop-blur-sm">
                    <span className="text-[10px] font-medium uppercase tracking-[0.08em] text-primary">{badgeLabel}</span>
                    <span className="h-1 w-1 rounded-full bg-primary/40" />
                    <span className="text-xs font-light tracking-tight text-foreground/80">{badgeText}</span>
                </div>

                <h1 ref={headerRef} className="max-w-2xl text-left text-5xl font-medium leading-[1.05] tracking-tight text-foreground font-display sm:text-6xl md:text-7xl">
                    {title}
                </h1>

                <p ref={paraRef} className="max-w-xl text-left text-base font-light leading-relaxed tracking-tight text-muted-foreground font-body sm:text-lg">
                    {description}
                </p>
                ...
                <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
                    {ctaButtons.map((button, index) => (
                        <a
                            key={index}
                            href={button.href}
                            className={`rounded-none border px-6 py-3 text-xs font-medium uppercase tracking-[0.2em] transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary/30 ${button.primary
                                ? "border-primary bg-primary text-white hover:bg-[#b08d4b] hover:shadow-lg hover:shadow-primary/10"
                                : "border-primary/20 text-foreground/80 hover:bg-primary/5 hover:border-primary/40 bg-transparent/50 backgrop-blur-sm"
                                }`}
                        >
                            {button.text}
                        </a>
                ...
                <ul ref={microRef} className="mt-8 flex flex-wrap gap-6 text-xs font-light tracking-tight text-muted-foreground/80 font-body">
